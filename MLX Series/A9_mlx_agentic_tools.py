...
def start_chat():
    while True:
        ...
        if user_input.lower() in {"exit", "quit", "bye"}:
            print("Goodbye!")
            break
        if user_input.lower() == "toggle deep search":
            global deep_search_mode
            deep_search_mode = not deep_search_mode
            print(f"Deep search mode {'enabled' if deep_search_mode else 'disabled'}.")
            continue
        ...
        if tool_call:
            result = execute_tool(tool_call["name"], tool_call["arguments"], user_input)
            print(f"[Tool]: {result}")
